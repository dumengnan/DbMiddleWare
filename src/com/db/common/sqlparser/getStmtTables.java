...
{
	//对数据库进行更新的表名存储在targetTable中
	private Set<String> targetTable = new LinkedHashSet<String>( );
	//select之后的表名 存储在sourceTable中
	private Set<String> sourceTable = new LinkedHashSet<String>( );
	...
	private void analyzeMergeStatement( TMergeSqlStatement merge )
	{
		if ( merge.getTargetTable( ) != null )
		{
			...
			targetTable.add(merge.getTargetTable().getFullName());
		}
	...
	private void analyzeDeleteStatement( TDeleteSqlStatement delete )
	{
		...
		if ( delete.getTargetTable( ) != null )
		{
			...
			targetTable.add(delete.getTargetTable( ).getFullName( ));
		}
	...
	private void analyzeUpdateStatement( TUpdateSqlStatement update )
	{
		if ( update.getTargetTable( ) != null )
		{
			...
			targetTable.add(update.getTargetTable( ).getFullName( ) );
		}
		Set<String> tables = new LinkedHashSet<String>( );
